...
For the `$29 Custom Local Print Pack Setup`, use the GitHub Pages service mirror as the public service path: `https://yanqr213.github.io/printable-tools-lab/custom-local-print-pack/`. The sales pack, structured GitHub issue form, payment-before-work reply template, fulfillment checklist, order-pipeline JSON, and manual outreach batch are published so an interested reply can become a paid external checkout order without collecting payment details in the repository. Count service revenue only after the external provider shows a paid order, payout balance, or settled payment.
