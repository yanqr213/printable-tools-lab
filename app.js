(function () {
  ...
  const landingPages = [
    ...
    },
    {
      ...
      tool: "invoice-followup-email",
      toolQuery: "invoiceStatus=sent&tone=warm&dueTiming=due-soon&context=I%20want%20to%20keep%20the%20message%20friendly%20and%20easy%20to%20act%20on.&paymentWording=Please%20use%20the%20payment%20link%20or%20invoice%20portal%20already%20sent.",
    ...
    {
      slug: "freelance-invoice-follow-up-email",
      ...
      tool: "invoice-followup-email",
      toolQuery: "invoiceStatus=sent&tone=friendly&dueTiming=due-soon&context=I%20want%20to%20check%20whether%20the%20client%20needs%20anything%20else%20from%20me%20before%20payment.&paymentWording=Please%20use%20the%20payment%20link%20or%20invoice%20portal%20already%20sent.",
      ...
      related: ["invoice-followup-email", "invoice-generator", "estimate-generator"],
      serviceLead: {
        serviceType: "invoice-followup-copy-pack",
    ...
    },
    {
      ...
      related: ["compress-image-to-kb", "resize-image", "crop-image", "passport-photo", "signature-png"],
    },
    {
    ...
    },
    {
      ...
      related: ["passport-photo", "compress-image-to-kb", "resize-image", "crop-image"],
    },
    {
      slug: "photo-200x230-50kb",
      ...
      tool: "resize-image",
      toolQuery: "width=200&height=230&fit=cover",
    ...
    },
    {
      ...
      related: ["compress-image-to-kb", "resize-image", "signature-png", "remove-background"],
    },
    {
      slug: "signature-under-50kb",
      ...
      related: ["compress-image-to-kb", "resize-image", "signature-png", "crop-image"],
    },
    {
      ...
      tool: "resize-image",
      toolQuery: "width=140&height=60&fit=contain",
    ...
    },
    {
      ...
      tool: "resize-image",
      toolQuery: "width=200&height=100&fit=contain",
  ...
  const sponsorExternalDiscoveryProof = {
    ...
    listedDirectories: [
      ...
      { name: "TechTools Launchpad compress PDF to 1MB listing", evidenceUrl: "https://techtools.cz/launchpad-api/tools/174" },
      { name: "TechTools Launchpad PDF under 1MB upload fix listing", evidenceUrl: "https://techtools.cz/launchpad-api/tools/175" },
      { name: "TechTools Launchpad photo under 100KB upload fix listing", evidenceUrl: "https://techtools.cz/launchpad-api/tools/176" },
      { name: "TechTools Launchpad image under 2MB upload fix listing", evidenceUrl: "https://techtools.cz/launchpad-api/tools/177" },
      { name: "TechTools Launchpad JPG under 200KB upload fix listing", evidenceUrl: "https://techtools.cz/launchpad-api/tools/178" },
      { name: "TechTools Launchpad resume PDF too large upload fix listing", evidenceUrl: "https://techtools.cz/launchpad-api/tools/179" },
      { name: "TechTools Launchpad PNG screenshot too large upload fix listing", evidenceUrl: "https://techtools.cz/launchpad-api/tools/180" },
  ...
  const sponsorOpsSubmissionQueue = [
    ...
    },
    {
      ...
      publicEmail: "",
      prospectId: "twinkl-teacher-resources",
      dealId: "guide-sponsor-pilot",
      vertical: "classroom-printable-sponsors",
      note: "Support route needs manual review before any sponsor or partnership note is submitted.",
    },
    {
      name: "Cloudmersive",
      ...
      note: "Highest route score, but real email, name, phone, and consent requirements keep this prepare-only until an authorized sender is available.",
    },
  ];
  ...
  function invoiceFollowupInlineLeadForm(options = {}) {
  ...
  function renderLandingPage(slug) {
    ...
    const serviceInvoiceAction = page.serviceLead?.serviceType === "upload-limit-fix-plan"
      ? `<a class="button secondary" data-service-invoice-jump data-track-event="service_invoice_request" data-track-tool="${escapeHtml(serviceLeadTrackTool(page.serviceLead.serviceType))}" href="${page.uploadErrorMatcher ? "#invoice-request" : "#service-request"}">Get a $9 upload fix plan</a> `
  ...
  function renderServiceUpgradeCta(tool) {
    ...
    if (isInvoiceFollowupTool(tool.id)) {
      ...
      const inlineForm = invoiceFollowupInlineLeadForm({
        path: `/tools/${tool.id}/`,
        utmSource: "tool_cta",
...
<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  ...
  async function loadOpsMetrics() {
    ...
    try {
      ...
      const servicePublicRequests = await loadServicePublicRequests();
      ...
      target.innerHTML = `
        ...
        ${servicePublicRequestCheckHtml(servicePublicRequests)}
      ...
      target.innerHTML = `
        ${uploadFixInvoiceCloseQueueHtml({}, {}, {})}
  ...
  async function loadServicePublicRequests() {
    try {
      const response = await fetch("/api/service-public-requests", { cache: "no-store" });
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error("Service public request evidence unavailable");
      return data;
    } catch (error) {
      return {
        ok: false,
        ...
        dataWarning: error.message || "Service public request evidence unavailable",
  ...
  function servicePublicRequestCheckHtml(servicePublicRequests) {
    if (!servicePublicRequests) return "";
    const quality = servicePublicRequests.dataQuality || "unknown";
    ...
    const publicCount = Number.isFinite(Number(servicePublicRequests.publicRequestCount)) ? Number(servicePublicRequests.publicRequestCount) : 0;
    const invoiceCount = Number.isFinite(Number(servicePublicRequests.invoiceFollowupRequestCount)) ? Number(servicePublicRequests.invoiceFollowupRequestCount) : 0;
    const paidCount = Number.isFinite(Number(servicePublicRequests.paidServiceRequestCount)) ? Number(servicePublicRequests.paidServiceRequestCount) : 0;
    const reviewCount = Number.isFinite(Number(servicePublicRequests.readyForReviewCount)) ? Number(servicePublicRequests.readyForReviewCount) : 0;
    const warning = servicePublicRequests.dataWarning ? `<p class="notice">${escapeHtml(servicePublicRequests.dataWarning)}</p>` : "";
    return `
      <div class="notice sponsor-lead-check service-public-request-check">
        <strong>Public-safe service request evidence</strong>
        <p>${escapeHtml(String(publicCount))} public GitHub service request issue(s), ${escapeHtml(String(invoiceCount))} invoice follow-up issue(s), ${escapeHtml(String(paidCount))} paid service issue(s), ${escapeHtml(String(reviewCount))} ready for review. Quality ${escapeHtml(quality)}.</p>
        <p><a href="/api/service-public-requests" target="_blank" rel="noreferrer">Open public-safe service request JSON</a> · <a href="${escapeHtml(sourceUrl)}" target="_blank" rel="noreferrer">Open GitHub evidence search</a></p>
        ${warning}
      </div>
    `;
  }
  ...
  function leadToPaymentCloseRows(data = {}, leadCheck = {}, serviceLeadCheck = {}, publicReplies = {}, servicePublicRequests = {}) {
    const totals = data.totals || {};
    ...
    return [
      {
        ...
        signal: `${localPrintRequests} lead(s), ${publicLocalPrintRequests} public issue(s), ${serviceIntent} request intent`,
        state: localPrintRequests ? "lead captured" : publicLocalPrintRequests ? "public request" : serviceIntent ? "intent only" : "waiting",
      ...
      },
      {
        ...
        signal: `${invoiceFollowupRequests} lead(s), ${publicInvoiceFollowupRequests} public issue(s), ${publicPaidServiceRequests} paid public issue(s), ${serviceIntent} shared service intent`,
      ...
      },
      {
        ...
        signal: `${auditRequests} audit lead(s), ${publicAuditRequests} public issue(s), ${auditIntent} audit intent`,
        state: auditRequests ? "audit follow-up" : publicAuditRequests ? "public request" : auditIntent ? "intent only" : "waiting",
        ...
        link: publicAuditRequests && !auditRequests ? publicSourceUrl : "/market-table-print-audit/",
      },
      {
        lane: "$9 seller kit",
        signal: `${sellerRequests} seller request(s), ${publicSellerRequests} public issue(s), ${sellerIntent} checkout intent`,
        state: sellerRequests ? "buyer requested" : publicSellerRequests ? "public request" : sellerIntent ? "intent only" : "waiting",
        ...
        link: publicSellerRequests && !sellerRequests ? publicSourceUrl : "/local-seller-starter-kit/",
      },
      {
  ...
  function leadToPaymentCloseHtml(data = {}, leadCheck = {}, serviceLeadCheck = {}, publicReplies = {}, servicePublicRequests = {}) {
    const rows = leadToPaymentCloseRows(data, leadCheck, serviceLeadCheck, publicReplies, servicePublicRequests);
    const activeRows = rows.filter((row) => !String(row.state).includes("waiting"));
    const urgentRows = rows.filter((row) => /lead|invoice|buyer|audit|public request/i.test(row.state));
    return `
      <section class="panel ops-lead-close-cockpit">
        ...
        <div class="metric-grid compact ops-project-grid">
          ...
          <div class="metric-tile"><strong>${rows.filter((row) => row.state === "lead captured" || row.state === "buyer requested" || row.state === "audit follow-up" || row.state === "invoice review" || row.state === "public request").length}</strong><span>needs reply</span></div>
  ...
  function uploadFixInvoiceCloseQueueHtml(data = {}, serviceLeadCheck = {}, servicePublicRequests = {}) {
    const totals = data.totals || {};
    const todayTotals = data.todayTotals || {};
    const serviceTypes = serviceLeadCheck?.serviceTypes || {};
    const uploadLeadCount = numberSignal(serviceTypes["upload-limit-fix-plan"]);
    ...
    const paymentReply = serviceLeadPaymentReplyCopy({ serviceType: "upload-limit-fix-plan", path: "/upload-limit-fix-plan/" });
    ...
    const publicSourceUrl = servicePublicRequests?.sourceUrl || "/api/service-public-requests";
    const warmRows = uploadFixInvoicePathRows(data);
    return `
      <section class="panel ops-upload-fix-invoice-close" data-upload-fix-invoice-close-queue>
        <div class="ops-project-head">
          <div>
          ...
          <div class="actions">
            ...
            <a class="button ghost" href="/upload-limit-fix-plan/">Open $9 page</a>
          </div>
        </div>
        <div class="metric-grid compact ops-project-grid">
          ...
          <div class="metric-tile"><strong>${serviceIntent}</strong><span>service intent</span></div>
          <div class="metric-tile"><strong>${todayServiceIntent}</strong><span>today intent</span></div>
          <div class="metric-tile"><strong>${checkoutConfigured ? "configured" : "missing"}</strong><span>checkout URL</span></div>
          <div class="metric-tile"><strong>paid_order_verified</strong><span>revenue gate</span></div>
        </div>
        <div class="ops-action-list">
          <article class="ops-action-card lead-close-card">
            <div>
        ...
        ${opsTable(["Upload path", "Views", "Today", "Intent", "Invoice requests"], warmRows.map((row) => [
          row.path,
          row.page_view || 0,
          row.today_page_view || 0,
  ...
  function uploadFixInvoicePathRows(data = {}) {
    const uploadPaths = [
    ...
    const project = (Array.isArray(data.projects) ? data.projects : []).find((item) => item.id === "printable-tools-lab") || {};
    const rowsByPath = new Map((Array.isArray(project.paths) ? project.paths : []).map((row) => [String(row.path || ""), row]));
    const rows = uploadPaths.map((path) => ({ path, ...(rowsByPath.get(path) || {}) }));
    return rows
      .sort((a, b) => uploadFixPathScore(b) - uploadFixPathScore(a))
      .slice(0, 8);
  }

  function uploadFixPathScore(row = {}) {
  ...
  function initialToolValues(tool) {
    ...
    if (tool.id === "passport-photo") {
      const params = new URLSearchParams(window.location.search || "");
      const preset = String(params.get("preset") || params.get("size") || "").toLowerCase();
      if (["us-passport", "uk-passport", "eu-35x45", "canada-passport", "australia-passport"].includes(preset)) values.preset = preset;
    }
