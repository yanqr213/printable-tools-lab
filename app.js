    ...
    return `
      <section class="shell section">
        ...
        <table class="event-table">
          ...
          <tbody>
            ${uploadLimitDecisions.map(([message, href, label, why]) => `<tr><td>${escapeHtml(message)}</td><td><a href="${escapeHtml(href)}" data-track-event="free_tool_depth" data-track-tool="site">${escapeHtml(label)}</a></td><td>${escapeHtml(why)}</td></tr>`).join("")}
          </tbody>
        </table>
        <div class="grid-3">
          ${uploadLimitShortcuts.map(([label, href, description]) => `<article class="tool-card"><h3>${escapeHtml(label)}</h3><p>${escapeHtml(description)}</p><a class="button" data-track-event="free_tool_depth" data-track-tool="site" href="${escapeHtml(href)}">Open fixer</a></article>`).join("")}
        </div>
      </section>
    `;
  }
