...
if (!fs.existsSync(appScriptFile)) failures.push("Missing app.js.");
  ...
  if (!script.includes("UPLOAD_FIX_FUNNEL_TOOL_IDS") || !script.includes("renderDownloadUploadFixAfterAction") || !script.includes("download-upload-fix-lead-form") || !script.includes("Send $9 upload check request") || !script.includes("I just downloaded") || !script.includes('data-utm-campaign="upload_limit_fix_plan"')) failures.push("Missing download success upload-limit fix-plan close CTA.");
