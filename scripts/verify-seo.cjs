...
if (!fs.existsSync(appScriptFile)) failures.push("Missing app.js.");
  ...
  if (!script.includes("Where should the external $9 invoice link go?") || !script.includes("This is where the external ${price} invoice link will be sent after fit is confirmed")) failures.push("Upload fix invoice forms should frame the one field as the external payment-link destination.");
...
if (!fs.existsSync(nosignuptoolsUploadBacklogScriptFile)) failures.push("Missing NoSignupTools upload-fix submission script.");
  ...
  if (!script.includes("PDF Under 2MB Upload Fix") || !script.includes("PDF Under 5MB Upload Fix") || !script.includes("Resume PDF Under 2MB Upload Fix") || !script.includes("Document Under 5MB Upload Fix") || !script.includes("Extract Text From PDF Without Uploading") || !script.includes("Merge PDF Without Uploading") || !script.includes("Split PDF Without Uploading") || !script.includes("Rotate PDF Pages Without Uploading") || !script.includes("Remove Pages From PDF Without Uploading") || !script.includes("Reorder PDF Pages Without Uploading") || !script.includes("Add Page Numbers to PDF") || !script.includes("Stamp PDF Without Uploading") || !script.includes("Sign PDF Without Uploading")) failures.push("NoSignupTools upload-fix submission script missing PDF/document limit payloads.");
}
