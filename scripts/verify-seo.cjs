...
if (!fs.existsSync(appScriptFile)) failures.push("Missing app.js.");
  ...
  if (!script.includes("UPLOAD_FIX_FUNNEL_TOOL_IDS") || !script.includes("renderDownloadUploadFixAfterAction") || !script.includes("download-upload-fix-lead-form") || !script.includes("Send $9 upload check request") || !script.includes("Open public-safe $9 request") || !script.includes("data-download-upload-fix-public-request") || !script.includes("I just downloaded") || !script.includes('data-utm-campaign="upload_limit_fix_plan"')) failures.push("Missing download success upload-limit fix-plan close CTA.");
  ...
  if (!script.includes("ensureServiceLeadContactCue") || !script.includes("data-service-lead-contact-cue") || !script.includes("One reply email or public handle unlocks") || !script.includes("serviceLeadPrivatePathLabel") || !script.includes("aria-invalid")) failures.push("Service lead forms should make the reply-contact value prop and no-contact recovery state visible.");
...
if (!fs.existsSync(uploadErrorCheatsheetFile)) failures.push("Missing upload error cheatsheet page.");
  ...
  if (!html.includes("Get $9 plan for this error") || !html.includes("data-upload-error-fix-plan") || !html.includes("data-upload-error-text=\"PDF must be under 1MB\"")) failures.push("Upload error cheatsheet missing row-level $9 fix-plan CTAs.");
