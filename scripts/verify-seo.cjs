...
for (const page of [
  ...
  if (!fs.existsSync(file)) {
    ...
    if (!html.includes('data-track-event="service_invoice_request"') || !html.includes("Request $19 invoice link")) failures.push(`${page.pathName} missing above-fold $19 invoice-link request CTA.`);
...
if (!fs.existsSync(appScriptFile)) failures.push("Missing app.js.");
  ...
  if (!script.includes("UPLOAD_FIX_FUNNEL_TOOL_IDS") || !script.includes("renderDownloadUploadFixAfterAction") || !script.includes("download-upload-fix-lead-form") || !script.includes("Send $9 upload check request") || !script.includes("data-download-upload-fix-invoice-request") || !script.includes("Open public-safe $9 request") || !script.includes("data-download-upload-fix-public-request") || !script.includes("I just downloaded") || !script.includes('data-utm-campaign="upload_limit_fix_plan"')) failures.push("Missing download success upload-limit fix-plan close CTA.");
  if (!script.includes("renderPdfToolUploadFixRequest") || !script.includes("data-compress-pdf-tool-fix-form") || !script.includes('data-utm-source="compress-pdf-tool"') || !script.includes("Send $9 target request") || !script.includes("uploadLimitCompressPdfToolSummary") || !script.includes("Portal target: PDF under") || !script.includes("pdfTargetLabel(targetSize")) failures.push("Compress PDF tool missing pre-download upload-limit fix-plan request path.");
  if (!script.includes("renderImageKbToolUploadFixRequest") || !script.includes("data-compress-image-kb-tool-fix-form") || !script.includes('data-utm-source="compress-image-kb-tool"') || !script.includes("Send $9 image target request") || !script.includes("uploadLimitImageKbToolSummary") || !script.includes("Portal target: image or photo under") || !script.includes('params.get("targetkb")')) failures.push("Compress image-to-KB tool missing pre-download upload-limit fix-plan request path.");
  ...
  if (!script.includes("data-upload-error-invoice-request") || !script.includes("Request $9 invoice link") || !script.includes('data-track-event="service_invoice_request"') || !script.includes("requestedNextStep")) failures.push("Upload error and service pages should expose explicit invoice-link request CTAs.");
...
if (!fs.existsSync(techtoolsUploadBacklogScriptFile)) failures.push("Missing TechTools upload-error backlog retry script.");
  ...
  if (!script.includes("Image Dimensions 600x600 Upload Fix") || !script.includes("PDF Not Accepted JPG Required Fix") || !script.includes("Email Attachment Too Large PDF Fix") || !script.includes("rateLimited") || !script.includes("techtools-upload-error-backlog-rate-limit.json")) failures.push("TechTools upload-error backlog retry script missing remaining high-intent listing payloads or rate-limit handling.");
}
...
if (!fs.existsSync(uploadErrorCheatsheetFile)) failures.push("Missing upload error cheatsheet page.");
  ...
  if (!html.includes('data-upload-error-invoice-request') || !html.includes('data-track-event="service_invoice_request"') || !html.includes("Request $9 invoice link")) failures.push("Upload error cheatsheet missing row-level $9 invoice-link request CTAs.");
  ...
  if (!html.includes("Open public-safe request") || !html.includes("data-upload-error-fix-plan") || !html.includes("data-upload-error-text=\"PDF must be under 1MB\"")) failures.push("Upload error cheatsheet missing row-level public-safe $9 fix-plan CTAs.");
  if (!html.includes('id="upload-error-quick-request"') || !html.includes('data-upload-error-quick-request') || !html.includes('data-utm-content="cheatsheet-row-quick"') || !html.includes("Send selected error request")) failures.push("Upload error cheatsheet missing row-level quick $9 request panel.");
...
if (!fs.existsSync(invoiceFollowupRouteFile)) failures.push("Missing invoice follow-up copy pack service route.");
  ...
  if (!html.includes("Request $19 invoice link") || !html.includes('data-track-event="service_invoice_request"') || !html.includes("Requested+next+step%3A+Request+external+%2419+checkout+or+invoice+link")) failures.push("Invoice follow-up route missing explicit external invoice-link request CTA.");
