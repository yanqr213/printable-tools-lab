...
if (!fs.existsSync(toolsJsonFile)) failures.push("Missing tools.json.");
  ...
  if (!Array.isArray(data.tools) || data.tools.length < 38) failures.push("tools.json missing tools.");
  if (!Array.isArray(data.guides) || data.guides.length < 73) failures.push("tools.json missing guides.");
...
if (!fs.existsSync(freePdfToolsFile)) failures.push("Missing free PDF tools directory page.");
  ...
  if (!html.includes("/tools/multi-image-pdf/") || !html.includes("/tools/merge-pdf/") || !html.includes("/tools/split-pdf/") || !html.includes("/tools/pdf-page-numbers/") || !html.includes("/tools/text-to-pdf/")) failures.push("Free PDF tools page missing conversion links.");
...
if (!fs.existsSync(finderFile)) failures.push("Missing PDF tool finder page.");
  ...
  if (!html.includes("/tools/image-to-pdf/") || !html.includes("/tools/merge-pdf/") || !html.includes("/tools/split-pdf/") || !html.includes("/tools/receipt-generator/")) failures.push("PDF tool finder missing high-intent tool links.");
...
if (!fs.existsSync(discoveryFile)) failures.push("Missing discovery.json.");
  ...
  if (!Array.isArray(discovery.landingPages) || discovery.landingPages.length < 20) failures.push("discovery.json missing high-intent landing pages.");
...
if (!fs.existsSync(docsToolsFile)) failures.push("Missing GitHub Pages discovery tools.json.");
  ...
  if (!Array.isArray(data.landingPages) || data.landingPages.length < 20) failures.push("GitHub Pages discovery tools.json missing high-intent landing pages.");
