...
function evaluateGates(local, live, searchConsole, discovery) {
  ...
  const productReady = local.toolCount >= 62
    && local.guideCount >= 91
    && local.landingPageCount >= 50
    && local.indexableRoutes >= 213
...
function missingProductReasons(local, live) {
  const reasons = [];
  if (local.toolCount < 62) reasons.push(`Only ${local.toolCount} tools found; target is 62 or more.`);
  if (local.guideCount < 91) reasons.push(`Only ${local.guideCount} guides found; target is 91 or more.`);
  if (local.landingPageCount < 50) reasons.push(`Only ${local.landingPageCount} high-intent landing pages found; target is 50 or more.`);
  if (local.indexableRoutes < 213) reasons.push(`Only ${local.indexableRoutes} indexable routes found; target is 213 or more.`);
